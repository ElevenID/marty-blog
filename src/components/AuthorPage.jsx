...
function AuthorPage() {
  ...
  const handoffAuthors = (characterProfile?.handoffAuthors || [])
    ...
    .map((id) => ({ id, name: BLOG_AUTHORS[id]?.name || id }));
          ...
          <Box sx={{ flex: 1 }}>
            ...
            {characterProfile?.archetype && (
              <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', mb: 1.5 }}>
                <Chip label={characterProfile.archetype} color="secondary" size="small" />
                {characterProfile.emotionalTemperature && (
                  <Chip label={characterProfile.emotionalTemperature} size="small" variant="outlined" />
                )}
              </Box>
            )}
        ...
        <Grid container spacing={3} sx={{ mb: 5 }}>
          <Grid item xs={12} md={7}>
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              <Typography variant="caption" fontWeight={700} color="text.secondary" sx={{ display: 'block', mt: 1.5, mb: 1, textTransform: 'uppercase', letterSpacing: 1 }}>
                Core question
              </Typography>
              <Typography
                variant="h6"
              ...
              {characterProfile.useWhen && (
                <Typography variant="body2" sx={{ lineHeight: 1.75 }}>
                  <strong>Best used when:</strong> {characterProfile.useWhen}
                </Typography>
              )}
            </Paper>
          </Grid>
          ...
          <Grid item xs={12} md={5}>
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', mb: 2.5 }}>
                {characterProfile.noticesFirst?.map((item) => (
              ...
              <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', mb: 2.5 }}>
                {characterProfile.voiceTraits?.map((item) => (
                  <Chip key={item} label={item} size="small" color="primary" variant="outlined" />
                ))}
              </Box>

              {handoffAuthors.length > 0 && (
                <>
                  <Typography variant="caption" fontWeight={700} color="text.secondary" sx={{ display: 'block', mb: 1, textTransform: 'uppercase', letterSpacing: 1 }}>
                    Best handoffs
                  </Typography>
                  <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap' }}>
                    {handoffAuthors.map(({ id, name }) => (
                      <Chip
                        key={id}
                        label={name}
                        size="small"
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  ...
                  <Box component="ul" sx={{ pl: 2.5, my: 0 }}>
                    {characterProfile.signatureMoves?.map((item) => (
                      <Typography key={item} component="li" variant="body2" color="text.secondary" sx={{ mb: 1, lineHeight: 1.75 }}>
                        {item}
                      </Typography>
                    ))}
                  </Box>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', mb: 2.5 }}>
                    {characterProfile.bestAssignments?.map((item) => (
                      <Chip key={item} label={item} size="small" variant="outlined" sx={{ bgcolor: 'background.paper' }} />
                    ))}
                  </Box>
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.8, mb: 2 }}>
                    {characterProfile.commercialFunction}
                  </Typography>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap' }}>
                    {characterProfile.favoriteExamples?.map((item) => (
                      <Chip key={item} label={item} size="small" variant="outlined" />
                    ))}
                  </Box>
                </Grid>
