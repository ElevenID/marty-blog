...
## Current normalization notes after recovery work

- Several supporting rows below now correspond to **live posts**, not net-new drafting ideas. Treat them as canonicalization / rename decisions where noted, not as permission to publish another colliding slug.
- Use `data-minimization-in-identity` as the canonical **blog** proof note in the privacy branch. Reserve `privacy-data-minimization` for the guide surface and treat the colliding live blog slug as a rename / retire candidate.
- Treat `why-the-marty-protocol-exists` as a live protocol-motivation insert between the category case and the full model-synthesis layer when the reader needs that bridge.
- Treat the operational guide/blog collisions as rename / retire work on the blog side before another large regeneration wave, not as automatic future-blog backlog.
...
| Priority | Working title | Working slug | Summary | Primary reader | Likely lead persona | Gravity well | Navigation role | Scene shape | Source anchors | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P2 | Privacy Data Minimization (legacy duplicate) | `privacy-data-minimization` | Legacy duplicate of the privacy proof note; keep it only as the guide-side source / rename-or-redirect review surface while `data-minimization-in-identity` remains the canonical blog destination. | Privacy and governance readers | Nora | Constraint Layer | Duplicate-slug review | 1 scene | privacy materials, presentation policy sources | Live post / duplicate-slug review |
| P2 | Selective Disclosure | `selective-disclosure` | Canonical mechanism explainer for the privacy branch, showing how selective disclosure works beneath `minimum-disclosure-is-a-policy-problem` and where wallet UX or SD-JWT-specific follow-ons should branch off. | Architect, privacy reader, implementer | Nora, Marcus | Constraint Layer | Deep dive / canonical anchor | 3 scenes | SD-JWT, ZK, presentation policy sources | Live anchor / overlap-review source |
| P2 | Selective Disclosure Explained (legacy merge surface) | `selective-disclosure-explained` | Legacy longer mechanism explainer; keep it only as a merge / redirect review surface while any uniquely stronger comparison detail is folded into `selective-disclosure`. | Privacy architect, implementer, skeptical buyer | Nora | Constraint Layer | Legacy merge review | 1 scene | `selective-disclosure`, SD-JWT, ZK, presentation policy sources | Live post / merge-review candidate |
| P2 | Deploying Marty for Airline Boarding | `deploy-airline-boarding` | Operational case study showing how Marty supports high-throughput, offline-sensitive travel verification environments where latency, resilience, and trust-anchor freshness matter. | Operator, travel architect, regulated buyer | Sofia, Aiko | Operational Proof | Use-case proof | 3 scenes | airline deployment guide, offline materials, ICAO context | Live post / guide-collision review |
| P2 | Deploying Marty for Age Verification | `deploy-age-verification` | Use-case piece showing how Marty supports yes/no age checks with minimum disclosure instead of over-collecting identity data. | Privacy-conscious buyer, retail architect | Nora, Sofia | Operational Proof | Use-case proof | 1 scene | age-verification context, privacy materials | Live post / guide-collision review |
| P2 | Deploying Marty for Enterprise Access | `deploy-enterprise-access` | Enterprise-focused piece showing how reusable credentials can span badge, network, and application access without fragmenting policy and trust logic. | Enterprise architect, security lead | Sofia, Elena | Operational Proof | Use-case proof | 3 scenes | enterprise deployment context, Cedar/governance materials | Live post / guide-collision review |
| P2 | Deploying Marty for Membership Credentials | `deploy-membership-credentials` | Broader credential case showing how Marty can support non-government membership or entitlement scenarios with portable trust and policy rules. | Product owner, ecosystem architect | Victor, Nora | Operational Proof | Use-case proof | 1 scene | deployment contexts, trust/policy sources | Live post / guide-collision review |
| P2 | Deployment Profiles in Practice | `deployment-profiles-in-practice` | Concrete operational walkthrough comparing self-hosted, hosted, and edge/kiosk deployment choices and what actually changes between them. | Operator, platform architect | Sofia | Operational Proof | Deep dive / bridge | 3 scenes | deployment profile sources, pricing docs | Live post / guide-collision review |
| P2 | Compliance Profiles in Practice | `compliance-profiles-in-practice` | Applied explainer showing how regulatory and standards obligations get translated into configured compliance behavior instead of custom-coded branching logic. | Compliance lead, regulated architect | Elena, Aiko | Constraint Layer | Deep dive | 3 scenes | compliance profile sources, EUDI materials | Live post / guide-collision review |
| P2 | Offline Verification Guide | `offline-verification-guide` | Runtime-focused guide to trust caches, freshness, revocation trade-offs, and failure handling when verification must continue without constant network access. | Operator, implementer | Sofia | Operational Proof | Deep dive / operator anchor | 3 scenes | offline verification materials, deployment sources | Live post / guide-collision review |
| P2 | Implementing OID4VCI with Marty | `impl-oid4vci` | Technical guide showing how OID4VCI issuance maps into Marty's objects and flows without collapsing the architecture into a transport-specific design. | Implementer | Daniel | Artifact and Adoption Proof | Implementation deep dive | 1 scene | issuance flows, protocol docs, schema sources | Live post / guide-collision review |
| P2 | Implementing OID4VP with Marty | `impl-oid4vp` | Technical guide showing how OID4VP presentation maps into trust, disclosure, and deployment decisions in the Marty model. | Implementer | Daniel | Artifact and Adoption Proof | Implementation deep dive | 1 scene | presentation flows, protocol docs, schema sources | Live post / guide-collision review |
...
| **Nora** | `Minimum Disclosure Is a Policy Problem`, `data-minimization-in-identity`, `selective-disclosure`, `deploy-age-verification`, `deploy-membership-credentials` | Nora keeps privacy boundaries visible and concrete across both flagship and supporting posts. |
...
- **Selective Disclosure** *(3 scenes)* — scene 1: the disclosure problem; scene 2: mechanism choices such as SD-JWT, predicates, and proofs; scene 3: policy and deployment implications, with handoffs into wallet UX or SD-JWT specifics when needed.
- **Selective Disclosure Explained (legacy merge surface)** *(1 scene)* — one editorial merge review that preserves only any uniquely useful mechanism-comparison table or phrasing before routing readers back to `selective-disclosure`.
